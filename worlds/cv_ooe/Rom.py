...
def patch_rom(world, rom, code_patch):
    ...
    rom.write_to_file(0x022EB2BE, "overlay_86", bytearray([world.options.death_link.value]))
    rom.write_to_file(0x022EB239, "overlay_86", bytearray([world.options.save_on_area_exit.value]))
    rom.write_to_file(0x022EB4D0, "overlay_86", bytearray([world.options.unlock_all_quests.value]))
    if world.options.randomize_stolen_glyphs == RandomStolenGlyphs.option_glyphsanity:
        rom.write_to_file(0x022EB2BC, "overlay_86", bytearray([0x01]))  # This is checked by Barlowe/Albus to spawn backup glyphs

    rom.write_to_file(0x022EB2BD, "overlay_86", bytearray([world.options.glyph_drop_multiplier.value]))
    ...
    rom.write_to_file(0x021E98BE, "overlay_0", bytearray(text_encoder(str(world.options.villagers_required.value).zfill(2))))  # Barlowe's dialogue in the bad ending
    ###############################################
...
class OoEPatchExtensions(APPatchExtension):
    ...
    def apply_modifiers(caller: APProcedurePatch, rom: bytes) -> bytes:
        ...
        glyph_drops = rom.read_from_file(0x022EB2BD, "overlay_86", 1)[0]
