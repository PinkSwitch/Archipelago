...
; Change 5 to 1 for Monica's quest
    ...
    .org 0x021FCA30
        .dw @Tex_BarloweNoDominus
        .dw @Tex_BarloweOneDominus
        .dw @Tex_BarloweTwoDominus
        .dw @Tex_BarloweUnion
...
@@Skip:
    b 0x02234854
;;;;;;;;;;;;;;;;;;;;;;
; Shows relevant text from Barlowe based on current items
@BarloweDialogueHandler:
    bl @CountDominusPieces
    cmp r0, 0
    beq @@BarloweTex_NoDominus
    cmp r0, 1
    beq @@BarloweTex_OneDominus
    cmp r0, 2
    beq @@BarloweTex_TwoDominus
    ; If the player has all three pieces of Dominus...
    mov r0, 0x73
    bl 0x020633F0
    cmp r0, 0 ; If the player has Dominus, but does NOT have Glyph Union
    beq @@BarloweTex_NoUnion
    mov r0, 0x0B0
    strh r0, [r13]
    mov r0, 2
    mov r1, 0
    mov r2, 06
    mov r3, 0xC0
    bl 0x0203AFD0 ; Teleport the player to reload this room
    bl 0x0203B014
    b 0x02237A6C
@@BarloweTex_NoDominus:
    ldr r1, =0x75C
    b @@End
@@BarloweTex_OneDominus:
    ldr r1, =0x075D
    b @@End
@@BarloweTex_TwoDominus:
    ldr r1, =0x075E
    b @@End
@@BarloweTex_NoUnion:
    ldr r1, =0x075F
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Returns count r0 of how many Dominus items we have
@CountDominusPieces:
    push r1,r2,lr
    mov r1, 0 ; Initialize count to zero
    mov r2, 0
@@Loop:
    mov r0, 0x32 ; Hatred
    cmp r2, 1
    addeq r0, r0, 1 ; Add 1 to get Anger
    cmp r2, 2
    moveq r0, 0x4F ; Agony
    push r1,r2
    bl 0x020633F0
...
@Tex_BarloweNoDominus:
    .db 0x01, 0x00, 0xE7, 0x02, 0xE3, 0x0A
    .db 0x27, 0x4F, 0x0C, 0x00, 0x33, 0x48, 0x41, 0x4E, 0x4F, 0x41, 0x01, 0xE6 ; Go, Shanoa!

    .db 0x39, 0x4F, 0x55, 0x00, 0x4D, 0x55, 0x53, 0x54, 0x00, 0x52, 0x45, 0x54, 0x52, 0x49, 0x45, 0x56
    .db 0x45, 0x00, 0x54, 0x48, 0x45, 0x00, 0x54, 0x48, 0x52, 0x45, 0x45, 0xE6 ; You must retrive the three

    .db 0x50, 0x49, 0x45, 0x43, 0x45, 0x53, 0x00, 0x4F, 0x46, 0x00, 0x3B, 0x24, 0x4F, 0x4D, 0x49, 0x4E
    .db 0x55, 0x53, 0x3D, 0x01, 0xE6, 0xE5, 0xE4, 0xEA ; Pieces of [Dominus]!
@Tex_BarloweOneDominus:
    .db 0x01, 0x00, 0xE7, 0x02, 0xE3, 0x0A
    .db 0x39, 0x4F, 0x55, 0x07, 0x56, 0x45, 0x00, 0x52, 0x45, 0x43, 0x4F, 0x56, 0x45, 0x52, 0x45, 0x44
    .db 0x00, 0x41, 0x00, 0x50, 0x49, 0x45, 0x43, 0x45, 0x00, 0x4F, 0x46, 0x00, 0x3B, 0x24, 0x4F, 0x4D
    .db 0x49, 0x4E, 0x55, 0x53, 0x3D, 0x01, 0xE6 ; You've recovered a piece of [Dominus]!

    .db 0x39, 0x4F, 0x55, 0x00, 0x4D, 0x55, 0x53, 0x54, 0x00, 0x48, 0x55, 0x52, 0x52, 0x59, 0x01, 0xE6 ; You must hurry!

    .db 0x34, 0x57, 0x4F, 0x00, 0x50, 0x49, 0x45, 0x43, 0x45, 0x53, 0x00, 0x59, 0x45, 0x54, 0x00
    .db 0x52, 0x45, 0x4D, 0x41, 0x49, 0x4E, 0x01, 0xE6, 0xE5, 0xE4, 0xEA ; Two pieces yet remain!

@Tex_BarloweTwoDominus:
    .db 0x01, 0x00, 0xE7, 0x02, 0xE3, 0x0A
    .db 0x2A, 0x55, 0x53, 0x54, 0x00, 0x4F, 0x4E, 0x45, 0x00, 0x50, 0x49, 0x45, 0x43, 0x45, 0x00, 0x4F
    .db 0x46, 0x00, 0x3B, 0x24, 0x4F, 0x4D, 0x49, 0x4E, 0x55, 0x53, 0x3D, 0x00, 0x52, 0x45, 0x4D, 0x41
    .db 0x49, 0x4E, 0x53, 0x0E, 0x0E, 0x0E, 0xE6 ; Just one piece of [Dominus] remains...

    .db 0x26, 0x49, 0x4E, 0x44, 0x00, 0x49, 0x54, 0x0C, 0x00, 0x53, 0x4F, 0x00, 0x54, 0x48, 0x41, 0x54
    .db 0x00, 0x57, 0x45, 0x00, 0x4D, 0x41, 0x59, 0x00, 0x44, 0x45, 0x4C, 0x49, 0x56, 0x45, 0x52, 0xE6 ; Find it, so that we may deliver

    .db 0x4D, 0x41, 0x4E, 0x4B, 0x49, 0x4E, 0x44, 0x00, 0x49, 0x54, 0x53, 0x00, 0x47, 0x52, 0x45, 0x41
    .db 0x54, 0x45, 0x53, 0x54, 0x00, 0x57, 0x49, 0x53, 0x48, 0x0E, 0x0E, 0x0E, 0xE6, 0xE5, 0xE4, 0xEA ; mankind its greatest wish...
@Tex_BarloweUnion:
    .db 0x01, 0x00, 0xE7, 0x02, 0xE3, 0x0B
    .db 0x37, 0x48, 0x41, 0x54, 0x00, 0x41, 0x52, 0x45, 0x00, 0x59, 0x4F, 0x55, 0x00, 0x57, 0x41, 0x49
    .db 0x54, 0x49, 0x4E, 0x47, 0x00, 0x46, 0x4F, 0x52, 0x1F, 0x01, 0xE6 ; What are you waiting for?!

    .db 0x28, 0x55, 0x52, 0x52, 0x59, 0x0C, 0x00, 0x41, 0x4E, 0x44, 0x00, 0x43, 0x41, 0x53, 0x54, 0x00
    .db 0x54, 0x48, 0x45, 0x00, 0x35, 0x4E, 0x49, 0x4F, 0x4E, 0x00, 0x27, 0x4C, 0x59, 0x50, 0x48, 0x01 ; Hurry, and cast the Union Glyph!
    .db 0xE6, 0xE5, 0xE4

    .db 0xE3, 0x0A, 0x0E, 0x0E, 0x0E, 0x37, 0x48, 0x45, 0x4E, 0x00, 0x59, 0x4F, 0x55, 0x00, 0x43, 0x41
    .db 0x4E, 0x0C, 0xE6 ;...When you can,

    .db 0x4F, 0x46, 0x00, 0x43, 0x4F, 0x55, 0x52, 0x53, 0x45, 0x0E, 0x0E, 0x0E, 0xE6, 0xE5, 0xE4 ; of course...

    .db 0xE7, 0x00, 0xE3, 0x01
    .db 0x08, 0x28, 0x45, 0x07, 0x53, 0x00, 0x43, 0x45, 0x52, 0x54, 0x41, 0x49, 0x4E, 0x4C, 0x59
    .db 0x00, 0x49, 0x4E, 0x00, 0x41, 0x00, 0x52, 0x55, 0x53, 0x48, 0x0E, 0x0E, 0x0E, 0xE6 ; (He's certainly in a rush...

    .db 0x30, 0x45, 0x52, 0x48, 0x41, 0x50, 0x53, 0x00, 0x29, 0x00, 0x53, 0x48, 0x4F, 0x55, 0x4C
    .db 0x44, 0x00, 0x43, 0x4F, 0x4D, 0x45, 0x00, 0x42, 0x41, 0x43, 0x4B, 0x00, 0x48, 0x45, 0x52
    .db 0x45, 0xE6 ; Perhaps I should come back here

    .db 0x41, 0x46, 0x54, 0x45, 0x52, 0x00, 0xE8, 0x07, 0x52, 0x45, 0x53, 0x43, 0x55, 0x49, 0x4E
    .db 0x47, 0x00, 0x4D, 0x4F, 0x52, 0x45, 0x00, 0x56, 0x49, 0x4C, 0x4C, 0x41, 0x47, 0x45, 0x52
    .db 0x53, 0xE8, 0x01, 0x00, 0x49, 0x4E, 0x53, 0x54, 0x45, 0x41, 0x44, 0x0E, 0x0E, 0x0E, 0x09 ; after <rescuing more villagers> instead...
