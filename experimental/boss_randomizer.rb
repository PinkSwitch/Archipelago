...
module BossRandomizer
  ...
  def get_randomized_boss_stat_updates(old_boss)
    stat_multiplier = named_rand_range_weighted(:boss_stat_mult_range)
    stat_updates = {}

    [
      ["HP", 9999, 1],
      ["MP", GAME == "dos" ? 0xFFFF : 0xFF, 1],
      ["SP", 0xFF, 1],
      ["AP", 0xFF, 1],
      ["EXP", 9999, 1],
      ["Attack", 0xFF, 1],
      ["Defense", 0xFF, 0],
      ["Physical Defense", 0xFF, 0],
      ["Magical Defense", 0xFF, 0],
    ].each do |attribute_name, max_value, min_value|
      next unless old_boss[attribute_name]

      adjusted_value = (old_boss[attribute_name] * stat_multiplier).round
      adjusted_value = max_value if adjusted_value > max_value
      adjusted_value = min_value if adjusted_value < min_value
      stat_updates[attribute_name] = adjusted_value
    end
  ...
  def change_boss_entities_according_to_mapping(boss_entities, old_boss_id_to_new_boss_id)
    ...
    boss_entities.shuffle(random: rng).each do |boss_entity|
      ...
      if options[:randomize_boss_stats]
        # Make the new boss have stat values based on the old boss so it fits in at this point in the game,
        # while still allowing a +/-50% variance so boss fights feel different.
        get_randomized_boss_stat_updates(old_boss).each do |attribute_name, new_value|
          queued_dna_changes[new_boss_id][attribute_name] = new_value
        end
      end
    end
